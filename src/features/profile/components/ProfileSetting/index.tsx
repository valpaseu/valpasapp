...
const ProfileSetting = () => {
  ...
  const settings = [
    { label: 'Schedule', icon: <MaterialIcons name="schedule" style={styles.icon} /> },
    { label: 'Earnings', icon: <MaterialIcons name="attach-money" style={styles.icon} /> },
    { label: 'Notifications', icon: <Ionicons name="ios-notifications" style={styles.icon} /> },
    { label: 'Messages', icon: <MaterialIcons name="message" style={styles.icon} /> },
    { label: 'Edit profile', icon: <AntDesign name="profile" style={styles.icon} /> },
    { label: 'Log out', icon: <FontAwesome name="sign-out" style={styles.icon} />, handlePress: handleLogOut },
  ]

  return (
    <View style={styles.container}>
      {settings.map(({ label, icon, handlePress = () => {} }, index) => (
        <View key={`profileSetting-${index}`} style={[styles.itemContainer, index === 0 && styles.firstItem]}>
